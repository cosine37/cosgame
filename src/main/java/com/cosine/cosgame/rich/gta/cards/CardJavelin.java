...
public class CardJavelin extends Card {
	public CardJavelin() {
		super();
		id = 47;
		name = "标枪卡";
		desc = "成功率：30%；+3starP，对一名玩家造成3点伤害。消耗。";
		rarity = 0;
	...
	public void play(int rawOptions) {
		...
		if (playable() && targetPlayerIndex>=0 && targetPlayerIndex<board.getPlayers().size()) {
			...
			if (f) {
				...
				board.getLogger().log(player.getName() + " 获得了 3 点通缉值");
				board.setBroadcastMsg(player.getName() + "使用了标枪卡，对" + targetName + "掷出标枪，" + tp.getName() + "受到了" + x + "点伤害，" + player.getName() + "获得了3点通缉值。");
				
				player.hurt(tp, x);
				player.addStar(3);
			} else {
				board.getLogger().log(player.getName() + " 但是没有命中 " + targetName);
				board.setBroadcastMsg(player.getName() + "使用了标枪卡，对" + targetName + "掷出标枪，但是并没有命中" + targetName +"。");
			}
