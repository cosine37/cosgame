...
public class Certify extends Event{
	public Certify() {
		...
		this.img = "certify";
		this.name = "勇者之证";
		this.desc = "返回营地时（在翻出下一张牌前），若有4张灾难被翻出，额外获得3枚钱币；若有5张或更多灾难被翻出，额外获得10枚钱币。";
	}
	...
	public boolean backHandle(Player p) {
		...
		if (x == 4) {
			p.addMoney(3);
			board.getLogger().log("二等勇者" + p.getName() + "收获了3枚金币作为奖金");
		} else if (x>=5) {
			p.addMoney(10);
			board.getLogger().log("一等勇者" + p.getName() + "收获了10枚金币作为奖金");
		}
		return false;
	}
}
